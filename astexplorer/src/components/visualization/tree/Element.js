...
const Element = React.memo(
  function Element({
    ...
    function renderChild(key, value, parent, name, computed) {
      if (
        treeAdapter.isArray(value) ||
        treeAdapter.isObject(value) ||
        typeof value === "function"
      ) {
        const ElementType =
          typeof value === "function" ? FunctionElement : ElementContainer;
        return (
          <ElementType
            key={key}
            name={name}
      ...
      if (typeof value.length === "number") {
        if (value.length > 0 && isOpen) {
          ...
          valueOutput = (
            <span>
              {valueOutput}
              <CompactArrayView array={value} onClick={onToggleClick} />
            </span>
          );
        }
...
const FunctionElement = React.memo(function FunctionElement(props) {
  ...
  if (computedValue !== NOT_COMPUTED) {
    if (
      treeAdapter.isArray(computedValue) ||
      treeAdapter.isObject(computedValue)
    ) {
      return (
        <ElementContainer
          {...props}
    ...
    return (
      <PrimitiveElement name={name} value={computedValue} computed={computed} />
    );
  }
