...
let reparse_ast =
  ...
  object (self)
    ...
    method tuple args =
      let args = List.map (fun (label, res) -> label, Result.get_ok res) args in
      Ok (Jsonoo.Encode.object_ (("type", Jsonoo.Encode.string "tuple") :: args))
    ...
    method int64 _ _ = Ok (Jsonoo.Encode.string "serializing int64 is not supported yet")
    method int32 _ _ = Ok (Jsonoo.Encode.string "serializing int32 is not supported yet")
    ...
    method list f l l' =
      ...
      Ok
        (Jsonoo.Encode.list
           (fun x -> x)
           (if l_length > l_length'
            then List.map2 (fun x x' -> Result.get_ok (f x x')) l l
            else if l_length < l_length'
            then List.map2 (fun x x' -> Result.get_ok (f x x')) l' l'
    ...
    method option f o o' =
      ...
      | Some x, Some x' ->
        let a = f x x' in
        a
      | None, None -> Ok (Jsonoo.Encode.object_ [ "type", Jsonoo.Encode.string "None" ])
      | _ -> Error "option"
    ...
    method! type_kind x x' =
      with_reparse (fun () -> super#type_kind x x') (fun () -> parse_ast#type_kind x)
    ...
    method! payload x x' =
      with_reparse (fun () -> super#payload x x') (fun () -> parse_ast#payload x)

    method! constant x x' =
      with_reparse (fun () -> super#constant x x') (fun () -> parse_ast#constant x)

    method! injectivity x x' =
      with_reparse (fun () -> super#injectivity x x') (fun () -> parse_ast#injectivity x)

    method! arg_label x x' =
      with_reparse (fun () -> super#arg_label x x') (fun () -> parse_ast#arg_label x)

    method! variance x x' =
      with_reparse (fun () -> super#variance x x') (fun () -> parse_ast#variance x)

    method! closed_flag x x' =
      with_reparse (fun () -> super#closed_flag x x') (fun () -> parse_ast#closed_flag x)
    ...
    method! rec_flag x x' =
      with_reparse (fun () -> super#rec_flag x x') (fun () -> parse_ast#rec_flag x)

    method! longident x x' =
      with_reparse (fun () -> super#longident x x') (fun () -> parse_ast#longident x)
  end
;;
...
let transform source kind =
  ...
  | Syntaxerr.Error e ->
    Error
      ("Syntax error at : "
       ^ Stdlib.Format.asprintf "%a" Location.print (Syntaxerr.location_of_error e))
;;
