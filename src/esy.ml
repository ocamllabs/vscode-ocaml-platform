...
module Package = struct
  ...
  let has_dependencies _t = failwith "Esy sandbox inspection is not supported yet."
...
module Discover = struct
  ...
  let is_esy_compatible filename json =
    ...
    || ((property_exists json "dependencies" || property_exists json "devDependencies")
...
let install t manifest ~packages =
  Cmd.Spawn (Cmd.append t ("-P" :: Path.to_string manifest :: "add" :: packages))
;;
...
let setup_sandbox t manifest =
  ...
  | Pending ->
    ...
    show_message `Info "Esy dependencies are not installed. Run esy under %s" root_dir
;;
