    ...
    | Ok () ->
      let+ res =
        ...
        (match Ocaml_lsp.is_version_up_to_date ocaml_lsp (ocaml_version_exn t) with
         | Ok () -> 
           Sandbox_info.update_lsp_status t.sandbox_info ~sandbox:t.sandbox ~lsp_status:`UpToDate
         | Error (`Msg msg) ->
           Sandbox_info.update_lsp_status t.sandbox_info ~sandbox:t.sandbox ~lsp_status:`OutOfDate;
           let (_ : unit Promise.t) =
...
module Sandbox_info : sig
  ...
  val update_lsp_status : StatusBarItem.t -> sandbox:Sandbox.t -> lsp_status:[`UpToDate | `OutOfDate] -> unit
end = struct
  ...
  let update_lsp_status sandbox_info ~sandbox ~lsp_status =
    let is_outdated = match lsp_status with `OutOfDate -> true | `UpToDate -> false in
    let status_bar_item_text = make_status_bar_item_text sandbox ~lsp_outdated:is_outdated () in
    StatusBarItem.set_text sandbox_info status_bar_item_text;
