...
object (self)
  ...
  method longident : longident -> 'res=
    fun x ->
      match x with
      | Lident a -> let a = self#string a in self#constr "Lident" [("label",a)]
      | Ldot (a, b) ->
          let a = self#longident a in
          let b = self#string b in self#constr "Ldot" [("longident",a); ("label",b)]
      | Lapply (a, b) ->
          let a = self#longident a in
          let b = self#longident b in self#constr "Lapply" [("longident1",a); ("longident2",b)]
  ...
  method arg_label : arg_label -> 'res=
    fun x ->
      ...
      | Labelled a -> let a = self#string a in self#constr "Labelled" [("label",a)]
      | Optional a -> let a = self#string a in self#constr "Optional" [("label",a)]
  ...
  method constant : constant -> 'res=
    fun x ->
      ...
      | Pconst_char a ->
          let a = self#char a in self#constr "Pconst_char" [("char",a)]
      | Pconst_string (a, b, c) ->
          let a = self#string a in
  ...
  method payload : payload -> 'res=
    fun x ->
      match x with
      | PStr a -> let a = self#structure a in self#constr "PStr" [("structure", a)] 
      | PSig a -> let a = self#signature a in self#constr "PSig" [("signature", a)]
      | PTyp a -> let a = self#core_type a in self#constr "PTyp" [("core_type",a)]
      | PPat (a, b) ->
  ...
  method core_type_desc : core_type_desc -> 'res=
    fun x ->
      ...
      | Ptyp_var a -> let a = self#string a in self#constr "Ptyp_var" [("label",a)]
      | Ptyp_arrow (a, b, c) ->
          let a = self#arg_label a in
          let b = self#core_type b in
          let c = self#core_type c in self#constr "Ptyp_arrow" [("arg_label",a); ("core_type1",b); ("core_type2",c)]
      | Ptyp_tuple a ->
      ...
      | Ptyp_object (a, b) ->
          ...
          let b = self#closed_flag b in self#constr "Ptyp_object" [("object_field list",a); ("closed_flag",b)]
      | Ptyp_class (a, b) ->
          let a = self#longident_loc a in
          let b = self#list self#core_type b in
      ...
      | Ptyp_alias (a, b) ->
          ...
          let b = self#string b in self#constr "Ptyp_alias" [("core_type",a); ("label",b)]
      | Ptyp_variant (a, b, c) ->
          let a = self#list self#row_field a in
          let b = self#closed_flag b in
          let c = self#option (self#list self#label) c in
          self#constr "Ptyp_variant" [("row_field list",a); ("closed_flag",b); ("label list option",c)]
      | Ptyp_poly (a, b) ->
          let a = self#list (self#loc self#string) a in
          let b = self#core_type b in self#constr "Ptyp_poly" [("label loc list",a); ("core_type",b)]
      | Ptyp_package a ->
          let a = self#package_type a in self#constr "Ptyp_package" [("package_type",a)]
      | Ptyp_extension a ->
          let a = self#extension a in self#constr "Ptyp_extension" [("extension",a)]
  ...
  method row_field_desc : row_field_desc -> 'res=
    fun x ->
      ...
      | Rinherit a ->
          let a = self#core_type a in self#constr "Rinherit" [("core_type",a)]
  ...
  method object_field_desc : object_field_desc -> 'res=
    fun x ->
      ...
      | Otag (a, b) ->
          let a = self#loc self#label a in
          let b = self#core_type b in self#constr "Otag" [("label loc",a); ("core_type",b)]
      | Oinherit a ->
          let a = self#core_type a in self#constr "Oinherit" [("core_type",a)]
  ...
  method pattern_desc : pattern_desc -> 'res=
    fun x ->
      ...
      | Ppat_var a ->
          let a = self#loc self#string a in self#constr "Ppat_var" [("label loc",a)]
      | Ppat_alias (a, b) ->
          let a = self#pattern a in
          let b = self#loc self#string b in self#constr "Ppat_alias" [("pattern",a); ("label loc",b)]
      | Ppat_constant a ->
          let a = self#constant a in self#constr "Ppat_constant" [("constant",a)]
      | Ppat_interval (a, b) ->
          let a = self#constant a in
          let b = self#constant b in self#constr "Ppat_interval" [("constant1",a); ("constant2",b)]
      | Ppat_tuple a ->
          let a = self#list self#pattern a in self#constr "Ppat_tuple" [("pattern list",a)]
      | Ppat_construct (a, b) ->
          let a = self#longident_loc a in
          let b = self#option self#pattern b in
          self#constr "Ppat_construct" [("longident_loc",a); ("pattern option",b)]
      | Ppat_variant (a, b) ->
      ...
      | Ppat_or (a, b) ->
          let a = self#pattern a in
          let b = self#pattern b in self#constr "Ppat_or" [("pattern1",a); ("pattern2",b)]
      | Ppat_constraint (a, b) ->
          let a = self#pattern a in
          let b = self#core_type b in self#constr "Ppat_constraint" [("pattern",a); ("core_type",b)]
      | Ppat_type a ->
          let a = self#longident_loc a in self#constr "Ppat_type" [("longident_loc",a)]
      | Ppat_lazy a ->
          let a = self#pattern a in self#constr "Ppat_lazy" [("pattern",a)]
      | Ppat_unpack a ->
          let a = self#loc (self#option self#string) a in
          self#constr "Ppat_unpack" [("label option loc",a)]
      | Ppat_exception a ->
          let a = self#pattern a in self#constr "Ppat_exception" [("pattern",a)]
      | Ppat_extension a ->
          let a = self#extension a in self#constr "Ppat_extension" [("extension",a)]
      | Ppat_open (a, b) ->
          let a = self#longident_loc a in
          let b = self#pattern b in self#constr "Ppat_open" [("longident_loc",a); ("pattern",b)]
  ...
  method expression_desc : expression_desc -> 'res=
    fun x ->
      match x with
      | Pexp_ident a ->
          let a = self#longident_loc a in self#constr "Pexp_ident" [("longident_loc",a)]
      | Pexp_constant a ->
          let a = self#constant a in self#constr "Pexp_constant" [("constant",a)]
      | Pexp_let (a, b, c) ->
          let a = self#rec_flag a in
          let b = self#list self#value_binding b in
          let c = self#expression c in self#constr "Pexp_let" [("rec_flag",a); ("value_binding list",b); ("expression",c)]
      | Pexp_function a ->
          let a = self#cases a in self#constr "Pexp_function" [("cases",a)]
      | Pexp_fun (a, b, c, d) ->
          let a = self#arg_label a in
          let b = self#option self#expression b in
          let c = self#pattern c in
          let d = self#expression d in self#constr "Pexp_fun" [("arg_label",a); ("expression option",b); ("pattern",c); ("expression",d)]
      | Pexp_apply (a, b) ->
      ...
      | Pexp_match (a, b) ->
          let a = self#expression a in
          let b = self#cases b in self#constr "Pexp_match" [("expression",a); ("cases",b)]
      | Pexp_try (a, b) ->
          let a = self#expression a in
          let b = self#cases b in self#constr "Pexp_try" [("expression",a); ("cases",b)]
      | Pexp_tuple a ->
      ...
      | Pexp_field (a, b) ->
          let a = self#expression a in
          let b = self#longident_loc b in self#constr "Pexp_field" [("expression",a); ("longident_loc",b)]
      | Pexp_setfield (a, b, c) ->
          let a = self#expression a in
          let b = self#longident_loc b in
          let c = self#expression c in
          self#constr "Pexp_setfield" [("expression1",a); ("longident_loc",b); ("expression2",c)]
      | Pexp_array a ->
          let a = self#list self#expression a in
      ...
      | Pexp_ifthenelse (a, b, c) ->
          ...
          self#constr "Pexp_ifthenelse" [("expression1",a); ("expression2",b); ("expression option",c)]
      | Pexp_sequence (a, b) ->
          let a = self#expression a in
          let b = self#expression b in self#constr "Pexp_sequence" [("expression1",a); ("expression2",b)]
      | Pexp_while (a, b) ->
          let a = self#expression a in
          let b = self#expression b in self#constr "Pexp_while" [("expression1",a); ("expression2",b)]
      | Pexp_for (a, b, c, d, e) ->
          let a = self#pattern a in
          let b = self#expression b in
          let c = self#expression c in
          ...
          self#constr "Pexp_for" [("pattern",a); ("expression1",b); ("expression2",c); ("direction_flag",d);("expression3",e)]
      | Pexp_constraint (a, b) ->
          let a = self#expression a in
          let b = self#core_type b in self#constr "Pexp_constraint" [("expression",a); ("core_type",b)]
      | Pexp_coerce (a, b, c) ->
          let a = self#expression a in
          let b = self#option self#core_type b in
          let c = self#core_type c in self#constr "Pexp_coerce" [("expression",a); ("core_type option",b); ("core_type",c)]
      | Pexp_send (a, b) ->
          let a = self#expression a in
          let b = self#loc self#label b in self#constr "Pexp_send" [("expression",a); ("label loc",b)]
      | Pexp_new a ->
          let a = self#longident_loc a in self#constr "Pexp_new" [("longident_loc",a)]
      | Pexp_setinstvar (a, b) ->
          let a = self#loc self#label a in
          let b = self#expression b in self#constr "Pexp_setinstvar" [("label loc",a); ("expression",b)]
      | Pexp_override a ->
          let a =
            self#list
              (fun (a, b) ->
      ...
      | Pexp_letmodule (a, b, c) ->
          ...
          let c = self#expression c in
          self#constr "Pexp_letmodule" [("label option loc",a); ("module_expr",b); ("expression",c)]
      | Pexp_letexception (a, b) ->
          let a = self#extension_constructor a in
          let b = self#expression b in
          self#constr "Pexp_letexception" [("Parsetree.extension_constructor",a); ("expression",b)]
      | Pexp_assert a ->
          let a = self#expression a in self#constr "Pexp_assert" [("expression",a)]
      | Pexp_lazy a ->
          let a = self#expression a in self#constr "Pexp_lazy" [("expression",a)]
      | Pexp_poly (a, b) ->
          let a = self#expression a in
          let b = self#option self#core_type b in
          self#constr "Pexp_poly" [("expression",a); ("core_type option",b)]
      | Pexp_object a ->
          let a = self#class_structure a in self#constr "Pexp_object" [("class_structure",a)]
      | Pexp_newtype (a, b) ->
          let a = self#loc self#string a in
          let b = self#expression b in self#constr "Pexp_newtype" [("label loc",a); ("expression",b)]
      | Pexp_pack a ->
          let a = self#module_expr a in self#constr "Pexp_pack" [("module_expr",a)]
      | Pexp_open (a, b) ->
          let a = self#open_declaration a in
          let b = self#expression b in self#constr "Pexp_open" [("open_declaration",a); ("expression",b)]
      | Pexp_letop a ->
          let a = self#letop a in self#constr "Pexp_letop" [("letop",a)]
      | Pexp_extension a ->
          let a = self#extension a in self#constr "Pexp_extension" [("extension",a)]
  ...
  method extension_constructor_kind : extension_constructor_kind -> 'res=
    fun x ->
      ...
      | Pext_decl (a, b) ->
          ...
          let b = self#option self#core_type b in
          self#constr "Pext_decl" [("constructor_arguments",a); ("core_type option",b)]
      | Pext_rebind a ->
          let a = self#longident_loc a in self#constr "Pext_rebind" [("longident_loc",a)]
  ...
  method class_type_desc : class_type_desc -> 'res=
    fun x ->
      ...
      | Pcty_arrow (a, b, c) ->
          let a = self#arg_label a in
          let b = self#core_type b in
          let c = self#class_type c in self#constr "Pcty_arrow" [("arg_label",a); ("core_type",b); ("class_type",c)]
      | Pcty_extension a ->
          let a = self#extension a in self#constr "Pcty_extension" [("extension",a)]
      | Pcty_open (a, b) ->
          let a = self#open_description a in
          let b = self#class_type b in self#constr "Pcty_open" [("open_description",a); ("class_type",b)]
  ...
  method class_type_field_desc : class_type_field_desc -> 'res=
    fun x ->
      match x with
      | Pctf_inherit a ->
          let a = self#class_type a in self#constr "Pctf_inherit" [("class_type",a)]
      | Pctf_val a ->
          let a =
            (fun (a, b, c, d) ->
               let a = self#loc self#label a in
      ...
      | Pctf_extension a ->
          let a = self#extension a in self#constr "Pctf_extension" [("extension",a)]
  ...
  method class_expr_desc : class_expr_desc -> 'res=
    fun x ->
      ...
      | Pcl_structure a ->
          let a = self#class_structure a in self#constr "Pcl_structure" [("class_structure",a)]
      | Pcl_fun (a, b, c, d) ->
          let a = self#arg_label a in
          let b = self#option self#expression b in
          let c = self#pattern c in
          let d = self#class_expr d in self#constr "Pcl_fun" [("arg_label",a); ("expression option",b); ("pattern",c); ("class_expr",d)]
      | Pcl_apply (a, b) ->
      ...
      | Pcl_let (a, b, c) ->
          let a = self#rec_flag a in
          let b = self#list self#value_binding b in
          let c = self#class_expr c in self#constr "Pcl_let" [("rec_flag",a); ("value_binding list",b); ("class_expr",c)]
      | Pcl_constraint (a, b) ->
          let a = self#class_expr a in
          let b = self#class_type b in self#constr "Pcl_constraint" [("class_expr",a); ("class_type",b)]
      | Pcl_extension a ->
          let a = self#extension a in self#constr "Pcl_extension" [("extension",a)]
      | Pcl_open (a, b) ->
          let a = self#open_description a in
          let b = self#class_expr b in self#constr "Pcl_open" [("open_description",a); ("class_expr",b)]
  ...
  method class_field_desc : class_field_desc -> 'res=
    fun x ->
      ...
      | Pcf_inherit (a, b, c) ->
          ...
          self#constr "Pcf_inherit" [("override_flag",a); ("class_expr",b); ("label loc option",c)]
      | Pcf_val a ->
          let a =
            (fun (a, b, c) ->
               let a = self#loc self#label a in
      ...
      | Pcf_attribute a ->
          let a = self#attribute a in self#constr "Pcf_attribute" [("attribute",a)]
      | Pcf_extension a ->
          let a = self#extension a in self#constr "Pcf_extension" [("extension",a)]
  method class_field_kind : class_field_kind -> 'res=
    fun x ->
      match x with
      | Cfk_virtual a ->
          let a = self#core_type a in self#constr "Cfk_virtual" [("core_type",a)]
      | Cfk_concrete (a, b) ->
          let a = self#override_flag a in
          let b = self#expression b in self#constr "Cfk_concrete" [("override_flag",a); ("expression",b)]
  ...
  method module_type_desc : module_type_desc -> 'res=
    fun x ->
      match x with
      | Pmty_ident a ->
          let a = self#longident_loc a in self#constr "Pmty_ident" [("longident_loc",a)]
      | Pmty_signature a ->
          let a = self#signature a in self#constr "Pmty_signature" [("signature",a)]
      | Pmty_functor (a, b) ->
          let a = self#functor_parameter a in
          let b = self#module_type b in self#constr "Pmty_functor" [("functor_parameter",a); ("module_type",b)]
      | Pmty_with (a, b) ->
      ...
      | Pmty_typeof a ->
          let a = self#module_expr a in self#constr "Pmty_typeof" [("module_expr",a)]
      | Pmty_extension a ->
          let a = self#extension a in self#constr "Pmty_extension" [("extension",a)]
      | Pmty_alias a ->
          let a = self#longident_loc a in self#constr "Pmty_alias" [("longident_loc",a)]
  method functor_parameter : functor_parameter -> 'res=
    fun x ->
      ...
      | Named (a, b) ->
          let a = self#loc (self#option self#string) a in
          let b = self#module_type b in self#constr "Named" [("label option loc",a); ("module_type",b)]
  ...
  method signature_item_desc : signature_item_desc -> 'res=
    fun x ->
      match x with
      | Psig_value a ->
          let a = self#value_description a in self#constr "Psig_value" [("value_description",a)]
      | Psig_type (a, b) ->
          let a = self#rec_flag a in
      ...
      | Psig_typext a ->
          let a = self#type_extension a in self#constr "Psig_typext" [("type_extension",a)]
      | Psig_exception a ->
          let a = self#type_exception a in self#constr "Psig_exception" [("type_exception",a)]
      | Psig_module a ->
      ...
      | Psig_open a ->
          let a = self#open_description a in self#constr "Psig_open" [("open_description",a)]
      | Psig_include a ->
      ...
      | Psig_attribute a ->
          let a = self#attribute a in self#constr "Psig_attribute" [("attribute",a)]
      | Psig_extension (a, b) ->
          let a = self#extension a in
          let b = self#attributes b in self#constr "Psig_extension" [("extension",a); ("attributes",b)]
  ...
  method with_constraint : with_constraint -> 'res=
    fun x ->
      ...
      | Pwith_module (a, b) ->
          let a = self#longident_loc a in
          let b = self#longident_loc b in self#constr "Pwith_module" [("longident_loc1",a); ("longident_loc2",b)]
      | Pwith_typesubst (a, b) ->
          let a = self#longident_loc a in
          let b = self#type_declaration b in
  ...
  method module_expr_desc : module_expr_desc -> 'res=
    fun x ->
      match x with
      | Pmod_ident a ->
          let a = self#longident_loc a in self#constr "Pmod_ident" [("longident_loc",a)]
      | Pmod_structure a ->
          let a = self#structure a in self#constr "Pmod_structure" [("structure",a)]
      | Pmod_functor (a, b) ->
          let a = self#functor_parameter a in
          let b = self#module_expr b in self#constr "Pmod_functor" [("functor_parameter",a); ("module_expr",b)]
      | Pmod_apply (a, b) ->
          let a = self#module_expr a in
          let b = self#module_expr b in self#constr "Pmod_apply" [("module_expr1",a); ("module_expr2",b)]
      | Pmod_constraint (a, b) ->
          let a = self#module_expr a in
          let b = self#module_type b in
          self#constr "Pmod_constraint" [("module_expr",a); ("module_type",b)]
      | Pmod_unpack a ->
          let a = self#expression a in self#constr "Pmod_unpack" [("expression",a)]
      | Pmod_extension a ->
          let a = self#extension a in self#constr "Pmod_extension" [("extension",a)]
  ...
  method structure_item_desc : structure_item_desc -> 'res=
    fun x ->
      ...
      | Pstr_eval (a, b) ->
          let a = self#expression a in
          let b = self#attributes b in self#constr "Pstr_eval" [("expression",a); ("attributes",b)]
      | Pstr_value (a, b) ->
          let a = self#rec_flag a in
          let b = self#list self#value_binding b in
      ...
      | Pstr_typext a ->
          let a = self#type_extension a in self#constr "Pstr_typext" [("type_extension",a)]
      | Pstr_exception a ->
          let a = self#type_exception a in self#constr "Pstr_exception" [("type_exception",a)]
      | Pstr_module a ->
          let a = self#module_binding a in self#constr "Pstr_module" [("module_binding",a)]
      | Pstr_recmodule a ->
      ...
      | Pstr_open a ->
          let a = self#open_declaration a in self#constr "Pstr_open" [("open_declaration",a)]
      | Pstr_class a ->
      ...
      | Pstr_attribute a ->
          let a = self#attribute a in self#constr "Pstr_attribute" [("attribute",a)]
      | Pstr_extension (a, b) ->
          let a = self#extension a in
          let b = self#attributes b in self#constr "Pstr_extension" [("extension",a); ("attributes",b)]
  ...
  method toplevel_phrase : toplevel_phrase -> 'res=
    fun x ->
      match x with
      | Ptop_def a ->
          let a = self#structure a in self#constr "Ptop_def" [("structure",a)]
      | Ptop_dir a ->
          let a = self#toplevel_directive a in self#constr "Ptop_dir" [("toplevel_directive",a)]
  ...
  method directive_argument : directive_argument -> 'res=
    fun { pdira_desc; pdira_loc } ->
      ...
      self#record "directive_argument" [("pdira_desc", pdira_desc); ("pdira_loc", pdira_loc)]
  method directive_argument_desc : directive_argument_desc -> 'res=
    fun x ->
      match x with
      | Pdir_string a ->
          let a = self#string a in self#constr "Pdir_string" [("label",a)]
      | Pdir_int (a, b) ->
          let a = self#string a in
          let b = self#option self#char b in self#constr "Pdir_int" [("label",a); ("char option",b)]
      | Pdir_ident a ->
          let a = self#longident a in self#constr "Pdir_ident" [("longident",a)]
      | Pdir_bool a -> let a = self#bool a in self#constr "Pdir_bool" [("bool",a)]
