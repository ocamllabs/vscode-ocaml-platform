...
module Experimental_capabilities = struct
  ...
  let t_of_json (json : Jsonoo.t) =
    ...
    let has_capability cap =
      Stdlib.Hashtbl.find_opt experimental_caps_tbl cap |> Option.value ~default:false
    in
    try
      ...
      let handleOcamlgrep = has_capability "handleOcamlgrep" in
    ...
    | Jsonoo.Decode_error err ->
      show_message
        `Warn
        "Unexpected experimental capabilities from the language server. Falling back to \
         default: no experimental capabilities set.";
  ...
  let main =
    [ ( (4, 12)
      , [| "1.5.0"
         ...
         ; "1.9.0"
        |] )
    ; (4, 13), [| "1.9.1"; "1.10.0"; "1.10.1"; "1.10.2"; "1.10.3"; "1.10.4"; "1.10.5" |]
    ; ( (4, 14)
...
let of_initialize_result (t : LanguageClient.InitializeResult.t) =
  ...
  let experimental_capabilities = Experimental_capabilities.of_initialize_result t in
...
let can_handle_ocamlgrep t = t.experimental_capabilities.handleOcamlgrep
