...
let get_repl_path () = Option.join (Settings.get ~section:"ocaml.repl" Repl_path.t)
...
let create_terminal instance sandbox =
  ...
  | None ->
    ...
    (match result with
     ...
     | Ok (Shell _) ->
       Promise.return (Error "Running a REPL from a Shell command is not supported")
     | Ok (Spawn cmd) ->
       ...
       let+ () =
         Promise.make (fun ~resolve ~reject:_ ->
           let (_ : Node.Timeout.t) = Node.setTimeout (fun () -> resolve ()) 2500 in
           ())
       in
...
let get_code text_editor =
  ...
  let start_char = Vscode.Selection.start selection |> Vscode.Position.character in
  ...
  else Vscode.TextDocument.getText document ~range:(selection :> Vscode.Range.t) ()
;;

let prepare_code code =
  if String.is_suffix (String.strip code) ~suffix:";;" then code else code ^ ";;"
;;
  ...
  let _open_repl =
    let handler (instance : Extension_instance.t) ~args:_ =
      let (_ : unit Promise.t) =
        ...
        let+ (result : (Terminal.t, string) result) = create_terminal instance sandbox in
  ...
  let disposable =
    Vscode.Window.onDidCloseTerminal
      ...
      ~listener:(fun closed_terminal ->
        ...
        | None -> ()
        | Some repl_terminal ->
          if phys_equal repl_terminal closed_terminal
          then Extension_instance.close_repl instance)
      ()
  in
