...
let remove_doc_entries (t : t) uri =
  let dirty_original_doc_set, origin_to_pp_doc_map =
    ...
    | Some uri ->
      Set.remove t.dirty_original_doc_set uri, Map.remove t.origin_to_pp_doc_map uri
    | None ->
...
let remove_dirty_original_doc t ~pp_uri =
  ...
  | Some uri -> t.dirty_original_doc_set <- Set.remove t.dirty_original_doc_set uri
  | None -> ()
;;
