module R = Result
let env_sep = match Sys.unix with | true -> ":" | false -> ";"
let propertyExists json property =
  let open Js.Json in
    match classify json with
    | ((JSONObject (json))[@explicit_arity ]) ->
        (match Js.Dict.get json property with
         | ((Some (j))[@explicit_arity ]) ->
             (match classify j with | JSONNull -> false | _ -> true)
         | None -> false)
    | _ -> false
let mapResultAndResolvePromise f r =
  let open Js.Promise in let open R in (r >>| f) |> resolve
...
module Fpath :
  ...
  struct
    let sep = match Sys.unix with | true -> "/" | false -> "\\\\"
    type t = string list
    ...
    then_
      (resolve <<
         (function
          ...
          | ((Error (e))[@explicit_arity ]) -> ((Error (e))
              [@explicit_arity ])))